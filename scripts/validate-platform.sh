...
echo "[2/5] YARN info"
docker compose exec -T resourcemanager bash -lc "curl -fsS http://resourcemanager:8088/ws/v1/cluster/info >/dev/null"

echo "[3/5] HiveServer2"
docker compose exec -T hive-server bash -lc "echo > /dev/tcp/127.0.0.1/10000"

echo "[4/5] Trino catalogs"
docker compose exec -T trino trino --server http://localhost:8080 --execute "SHOW CATALOGS;" >/dev/null
